...
import { User, Menu, Home, Car, Calendar, HelpCircle, Info, X } from "lucide-react";
...
const Navbar = () => {
    ...
    <>
      {menuItems.map((item) => (
        <NavigationMenuItem key={item.label}>
          <Link to={item.path}>
            <div className="flex items-center gap-2 px-4 py-2 text-white hover:bg-gray-800 rounded-md transition-all duration-200">
        ...
        <div className="flex flex-col pt-8">
          {menuItems.map((item) => (
            <SheetClose asChild key={item.label}>
              <Link to={item.path}>
                <div className="py-6 px-6 flex items-center hover:bg-gray-800 transition-colors">
  ...
  return (
    <nav className="bg-black py-3 px-6 sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex items-center space-x-2">
          {isMobile ? (
            ...
            <div className="flex items-center space-x-2">
              {isAuthenticated ? (
                <Link to="/dashboard">
                  <Button variant="default" size="sm" className="bg-white text-black hover:bg-gray-200 transition-colors">
                    Dashboard
                  </Button>
                </Link>
              ) : (
