...
const MobileMenu = ({ 
  ...
  const isDriverRoute = location.pathname.startsWith('/drive') || 
      ...
      <SheetContent side="right" className="w-full h-full p-0 m-0 max-w-full bg-black text-white border-none">
        <div className="flex flex-col h-full">
          ...
          <div className="flex justify-between items-center py-4 px-6 border-b border-gray-700">
            <Link to={isDriverRoute ? "/drive" : "/"} onClick={() => setIsMenuOpen(false)}>
              <span className="text-white font-bold text-2xl tracking-tight">
                {isDriverRoute ? "Uniride Driver" : "Uniride"}
              </span>
            </Link>
            <SheetClose asChild>
          ...
          <div className="flex-1 pt-8">
            ...
            {isAuthenticated && (
              <SheetClose asChild>
                <Link to={isDriverRoute ? "/driver-dashboard" : "/dashboard"}>
                  <div className="py-6 px-6 flex items-center hover:bg-gray-900">
